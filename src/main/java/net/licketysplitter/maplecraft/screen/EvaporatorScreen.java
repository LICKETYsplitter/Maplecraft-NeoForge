...
public class EvaporatorScreen extends AbstractContainerScreen<EvaporatorMenu> {
    ...
    private void renderEmptyBucket(GuiGraphics guiGraphics, int x, int y){
        if(menu.isBucketEmptying()){
            guiGraphics.blit(RenderPipelines.GUI_TEXTURED, TEXTURE, x + 18, y + 37, 176, 14, 14, menu.getBucketScaledProgress(), 256, 256);
        }
    }
    private void renderBottleFill(GuiGraphics guiGraphics, int x, int y){
        if(menu.isBottleFilling()){
            guiGraphics.blit(RenderPipelines.GUI_TEXTURED,TEXTURE, x + 144, y + 37, 176, 14, 14, menu.getBottleScaledProgress(), 256, 256);
        }
    }
    ...
    private void renderLitProgress(GuiGraphics guiGraphics, int x, int y){
        if(menu.isLit()){
            ...
            guiGraphics.blit(RenderPipelines.GUI_TEXTURED,TEXTURE, x + 80, y + 47 + progress, 176, progress, 14, 14 - progress, 256, 256);
        }
    }
    ...
    private void renderPanProgress(GuiGraphics guiGraphics, int x, int y, int panID){
        if(menu.panHasContents(panID)){
            ...
            guiGraphics.blit(RenderPipelines.GUI_TEXTURED, TEXTURE, x + 53 + (18 * panID), y + 14 + (31 - fullness),
                    176, 29, 16, fullness, 256, 256);

            /*SAP*/
            //guiGraphics.setColor(1.0f, 1.0f, 1.0f, (1.0f - progress));
            guiGraphics.blit(RenderPipelines.GUI_TEXTURED,TEXTURE, x + 53 + (18 * panID), y + 14 + (31 - fullness),
                    176, 60, 16, fullness, 256, 256);
